...
output_path = Path('../output/friends_approval_analysis.csv')
...
def process_ego(ego_path, dicowriter):
    ...
    try:
        ...
        for row in dicoreader:
            ...
            if diff_month(tmp_month, row_month, tmp_year, row_year) == 1 and mk_int(row['approved_friends']) >= 20:
                count_thres += 1
                ...
                if count_thres >= 2:
                    ...
                    dicowriter.writerow(
                        {'id_ego': id_ego, 'date_begin': date_begin, 'date_end': date_end, 'month_diff': delta_month,
                         'nb_total': total_in_thres, 'type': 'approvedfriends', 'thresold': 2, 'smoothing': 20})
                count_thres = 0
                total_in_thres = 0
...
def process_egos(filespath, dicowriter):
    for file in filespath:
        process_ego(file, dicowriter)
...
def get_csvwriter():
    csvf = output_path.open('a', encoding='utf-8')
    dicowriter = csv.DictWriter(
        csvf, fieldnames=['id_ego', 'date_begin', 'date_end', 'month_diff', 'nb_total', 'type', 'thresold', 'smoothing'],
        delimiter=',')
...
if __name__ == '__main__':
    ...
    dicowriter, file = get_csvwriter()
    process_egos(files_path, dicowriter)
    file.close()
